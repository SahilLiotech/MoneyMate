    ...
    //check if password is available in the database or not
        fun isOldPasswordCorrect(uid: String, oldPassword: String): Boolean {
            val db = readableDatabase
            val query = "SELECT $COLUMN_ID FROM $TABLE_NAME WHERE $COLUMN_ID = ? AND $COLUMN_PASSWORD = ?"
            val cursor = db.rawQuery(query, arrayOf(uid, oldPassword))
...
//update the password
    fun updateUserPassword(uid:String, newPassword: String): Int {
        val db = writableDatabase
        val values = ContentValues().apply {
            put(COLUMN_PASSWORD, newPassword)
        }
        return db.update(TABLE_NAME, values, "$COLUMN_ID=?", arrayOf(uid))
      }
    }